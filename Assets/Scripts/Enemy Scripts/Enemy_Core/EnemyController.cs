...
namespace Moving_Tower
{
    public class EnemyController : MonoBehaviour, IEnemyObject
    {
        ...
        public float waypointDiff = 0.4f;
        ...
        public virtual void OnObjectSpawn(byte wayPointIndex)
        {
            ...
            speedMult = 1;
            waypointDiff = 0.4f;
            //Debug.Log($"waypoint 0 : {waypoints.wayPoints[_wayPointCount]}");
        ...
        {
            transform.Translate(targetDir.normalized * enemyStats.speed * speedMult * Time.deltaTime);

            //if (Vector3.Distance(transform.position, waypoints.wayPoints[_wayPointCount]) <= 0.4f)
            if ((_wayPointCount != 2 && Mathf.Abs(transform.position.z - waypoints.wayPoints[_wayPointCount].z) <= waypointDiff)
                || (_wayPointCount == 2 && Mathf.Abs(transform.position.x - waypoints.wayPoints[_wayPointCount].x) <= waypointDiff))
                UpdateWayPoint();
        ...
        private void UpdateWayPoint()
        {
            if (_wayPointCount < waypoints.wayPoints.Count - 1)
            {
                _wayPointCount++;
                targetDir = waypoints.wayPoints[_wayPointCount] - transform.position;
                //Debug.Log($"Updating Waypoint : {waypoints.wayPoints[_wayPointCount]}");
            }
            else
            {
                //Debug.Log($"Reached Castle");
                if (!GameManager.instance.enemyReachedCastle)       //Check if any other unit has already reached castle or not?
                {
                    GameManager.instance.enemyReachedCastle = true;
        ...
        //No need to reset position as the Enemy spawner can handle that at the time of spawning
